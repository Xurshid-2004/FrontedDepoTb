...
import { fio, fmtDT, jonliRang } from "@/lib/logic";
import type { FeatureKey, Perm } from "@/lib/permissions";
import { Badge, Btn } from "./ui";
import { NavIcon } from "./NavIcons";

/** Har menyu boʻlimi uchun oʻz rangi — belgilar rang-barang jilolanadi,
 *  aktiv boʻlim shu rangda ajralib turadi. */
const NAV_RANG: Record<string, string> = {
  "/dash": "#1d4ed8",
  "/tb": "#0d9488",
  "/arizalar": "#ea580c",
  "/ombor": "#7c3aed",
...
const navRang = (href: string) => NAV_RANG[href] ?? "#475569";
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const navAll: NavItem[] = [
    ...
    { href: "/tibbiy", label: "Tibbiy koʻrik", perm: "tibbiy.read", feat: "nav.tibbiy" },
    { href: "/psixolog", label: "Psixolog", perm: "psixolog.read", feat: "nav.psixolog" },
            ...
            {nav.map((n) => {
              ...
              return (
                <Link
                  ...
                  className={`group relative flex items-center gap-3 rounded-xl px-3 py-3 text-[14px] font-semibold transition-all ${
                    active ? "font-bold" : "text-slate-700 hover:bg-slate-100 hover:text-slate-900"
                  }`}
                >
                  {active && (
                    <motion.span
                  ...
                  <span className="flex-1" style={{ color: c }}>{n.label}</span>
                  {!!n.badge && (
                    <span
          ...
          {nav.map((n) => {
            ...
            return (
              <Link
                ...
                title={n.label}
                aria-label={n.label}
                style={active ? { background: `${c}1f`, color: c, boxShadow: `inset 0 0 0 1px ${c}66` } : { color: c }}
                className="relative grid h-11 w-11 shrink-0 place-items-center rounded-xl transition hover:bg-slate-100"
              >
          ...
          <header className="flex h-[var(--header-h)] items-center gap-2 border-b border-slate-200 bg-white/90 px-3 backdrop-blur-xl md:gap-3 md:px-7">
            ...
            <div className="relative shrink-0">
              <button
                ...
                className={`relative grid h-11 w-11 place-items-center rounded-xl border-2 transition active:scale-95 md:h-10 md:w-10 ${
                  unread > 0
                    ? "border-sky-500 bg-sky-50 text-sky-600"
                    : "border-slate-200 text-slate-600 hover:border-sky-500 hover:text-sky-600"
                }`}
              >
                <svg
                  width="22"
                ...
                {unread > 0 && (
                  <span className="absolute -right-1.5 -top-1.5 grid h-5 min-w-5 place-items-center rounded-full bg-red-500 px-1 text-[11px] font-bold text-white shadow ring-2 ring-white">
                    {unread}
                  </span>
                )}
              </button>
              <AnimatePresence>
                {notifOpen && (
                  <>
                    ...
                    >
                      <p className="mb-2.5 px-1 text-[13px] font-bold uppercase tracking-wide text-slate-700">
                        Bildirishnomalar
                      </p>
                      {myNotifs.length === 0 && (
                      ...
                      {myNotifs.map((n) => {
                        // Har bildirishnoma oʻz rangida ajralib turadi (id boʻyicha
                        // barqaror-tasodifiy). Rad etilgan xabar doim qizil.
                        const rangi = n.turi === "reject" ? "#dc2626" : jonliRang(n.id);
                        return (
                          <div
                            key={n.id}
                            className="mb-2 rounded-xl border-l-4 px-3 py-3 transition"
                            style={{ borderLeftColor: rangi, background: `${rangi}12` }}
                          >
                            <div className="flex items-start justify-between gap-2">
                              <p className="text-[16px] font-bold" style={{ color: rangi }}>{n.sarlavha}</p>
                              <div className="flex shrink-0 items-center gap-1.5">
                                {n.turi === "reject" && <Badge color="#dc2626">rad</Badge>}
                                {!n.oqilgan && (
                                  <span
                                    className="mt-1 h-3 w-3 rounded-full"
                                    style={{ background: rangi }}
                                    aria-label="oʻqilmagan"
                                  />
                                )}
                              </div>
                            </div>
                            <p className="mt-1 text-[16px] leading-relaxed text-slate-700">{n.matn}</p>
                            <p className="mt-1.5 text-[12.5px] font-semibold" style={{ color: `${rangi}cc` }}>{fmtDT(n.sana)}</p>
                          </div>
                        );
                      })}
                    </motion.div>
              ...
              >
                <span className="relative">
                  ...
                  {!!n.badge && (
                    <span
                      style={{ background: c }}
                      className="absolute -right-2 -top-1.5 grid h-4 min-w-4 place-items-center rounded-full px-1 text-[9px] font-bold text-white"
                    >
                      {n.badge}
                    </span>
                  )}
                ...
                {active && (
                  <span className="absolute inset-x-4 top-0 h-[2.5px] rounded-b-full" style={{ background: c }} />
                )}
              </Link>
            );
          })}
